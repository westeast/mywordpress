...
<?php while ( have_posts() ) : the_post() ?>
			<div id="post-<?php the_ID() ?>" class="<?php sandbox_post_class() ?>">
                ...
                <?php if ( function_exists('wp_list_comments') ) { // WP 2.7 check 
					if (is_sticky() && !is_paged()) { ?>
						<img style="float:none" src="<?php bloginfo('template_url'); ?>/images/sticky.gif" alt="" title="Sticky Post!" /><?
					}
				} ?>
                
                <a href="<?php the_permalink() ?>" title="<?php printf(__('Permalink to %s', 'sandbox'), wp_specialchars(get_the_title(), 1)) ?>" rel="bookmark"><?php the_title() ?></a></h2>
